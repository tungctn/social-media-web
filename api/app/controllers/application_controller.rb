class ApplicationController < ActionController::Base
  ...
  # Lấy dữ liệu chi tiết của nhiều bài viết theo where query
  def get_list_post_data_by_where_query(where_query = "")
    posts = nil

    if !params[:page_index] || !params[:page_size]
      posts = Post.where(where_query).order(created_at: :desc)
    else
      skip = params[:page_size].to_i * (params[:page_index].to_i - 1)
      posts = Post.limit(params[:page_size].to_i).offset(skip).where(where_query).order(created_at: :desc)
    end
